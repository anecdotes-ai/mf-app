...
@Injectable()
export class RiskCreatedHandler implements EventHandler<PusherMessage> {
  readonly messageType = PusherMessageType.RiskCreated;

  constructor(private store: Store) {}

  handle(message: PusherMessage): void {
    if (message?.message_object?.updated) {
      this.store.dispatch(RisksActions.RiskAdded({ risk: message.message_object.updated }));
    }
  }
}
